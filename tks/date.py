...
__TargetType = namedtuple('TargetType', 'SQUARE RECTANGLE CIRCLE')
TargetType = __TargetType('square', 'rectangle', 'circle')
...
class DateEntry(ttk.Frame, object):
    ...
    :returns: The selected date as a datetime or date depending on the
              class used as `start_date`
    
    Dates will always consist of a 4 digit year and 2 digit months and days
    only the order and separator are determined by `locale`
    """
    ...
    def __init__(self, master,
        ...
        btn = ttk.Button(self, text=_('Select...'), command=self.select_date)
...
class DateSelector(ttk.Frame, object):
    ...
    def month(self, value):
        self.date = datetime.date(self._date.year,
                                  value,
                                  self._date.day)
    ...
    def _year_selected(self):
        ...
        self._prev_selector.grid(row=1, column=0, sticky=(tk.N, tk.EW), padx=3, pady=3)

    def _new_month_selected(self, date_):
        self._date = date_
        self._ms.date = date_
        self._ys.date = date_
...
class DaySelector(ttk.Frame, object):
    ...
    def __init__(self, master,
        ...
        self._header_color = kwargs.get('header_color', TksColors.Header)
        self._select_color = kwargs.get('select_color', TksColors.Select)
        self._today_color = kwargs.get('today_color', self.COLOR_FILL_TODAY)
        self._other_month_color = kwargs.get('other_month_color', self.COLOR_TEXT_OTHER_MONTH)
        ...
        self._calendar = calendar.TextCalendar(self._first_week_day)
        ...
        self._month_btn.bind('<ButtonRelease-1>', self._master._month_btn_clicked)
        
        self._year_btn = ttk.Button(self._header,
                                   style='Selector.tks.TButton')
        self._year_btn.grid(row=0, column=2, sticky=tk.EW, padx=(2, 0))
        self._year_btn.bind('<ButtonRelease-1>', self._master._year_btn_clicked)
    ...
    def _create_canvas(self, target_type):
        ...
        if target_type == TargetType.CIRCLE:
            ...
            item_width = item_height = max(item_width, item_height, circle_diameter)
        ...
        self._canvas.configure(width=x_pos - (x_stride / 2), height=y_pos - (y_stride / 2))
    ...
    def _next_month(self):
        ...
        self._master._new_month_selected(self._date)
        self._update_canvas()
...
class MonthSelector(ttk.Frame, object):
    ...
    def __init__(self, master, locale):
        ...
        btn_frame.grid(row=1, column=0, columnspan=3, pady=(4, 0), sticky=tk.NSEW)
...
class YearSelector(ttk.Frame, object):
    def __init__(self, master):
        ...
        self._year_btn.grid(row=0, column=1, sticky=tk.EW)
        #self._year_btn.bind('<Button-1>', self._year_btn_clicked)
